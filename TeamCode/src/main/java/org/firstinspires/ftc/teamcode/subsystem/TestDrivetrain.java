...
public class TestDrivetrain {
    ...
    public TestDrivetrain(ExpansionHubMotor m0, ExpansionHubMotor m1, ExpansionHubMotor m2, ExpansionHubMotor m3,
                          Telemetry telemetry, BNO055IMU imu, ExpansionHubEx chub, ExpansionHubEx ehub) {
        this.m0 = m0;
        this.m1 = m1;
        this.m2 = m2;
        this.m3 = m3;
    ...
    public void goToPoint(Vector2d point, double targetHeading, double power) {
        double target_angle = point.getAngle();
        updateSwerve(targetHeading, target_angle, power);
    }

    public void updateSwerve(double rotAngle, double transAngle, double transPow) {
        ...
        edata = ehub.getBulkInputData();
        cdata = chub.getBulkInputData();
...
//        double rotPower = rotVec.magnitude();
        double rotTheta = rotAngle;

        double transPower = Range.clip(transPow, 0.0, 1.0);
        double transTheta = Math.toDegrees(transAngle);

        //current robot heading
        double rotPhi = getIMUAngle();
